...
- Name: self-service-registration.spec.ts >> Self-Service Registration Flow >> should allow user to register and create mosque in trial mode
- Location: e2e\self-service-registration.spec.ts:4:7
...
Locator: locator('text=Inscription réussie')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=Inscription réussie')
...
- main:
  ...
  - text: auth.name *
  - textbox "auth.name *": Test User
  - text: auth.email *
  - textbox "auth.email *": test1783796025284@example.com
  - text: auth.password *
  - textbox "auth.password *": TestPassword123
  - paragraph: Minimum 8 caractères
  - heading "Informations de la mosquée" [level=3]
  - text: Nom de la mosquée *
  - textbox "Nom de la mosquée *": Test Mosque
  - text: Ville *
  - textbox "Ville *": Conakry
  - text: Pays *
  - textbox "Pays *": Guinea
  - paragraph:
