  ...
  return (
    <form onSubmit={handleSubmit} className="w-full space-y-3 rounded-md border border-gray-200 p-3 bg-gray-50">
      ...
      <div className="space-y-1">
        <Label htmlFor={`name-${userId}`} className="text-xs">Nom</Label>
      ...
      <div className="space-y-1">
        ...
        <Input id={`email-${userId}`} name="email" type="email" required defaultValue={email} className="h-8 text-sm" />
      </div>

      <div className="space-y-1">
        <Label htmlFor={`role-${userId}`} className="text-xs">Rôle</Label>
        <select
          id={`role-${userId}`}
          name="role"
